function Ekkp1 = E_k_kp1_rep2_normalized(n, d, lambda, k)
    % Function to compute the normalized matrix representation of E_{k,k+1}
    % Inputs:
    ...
    % Output:
    %   Ekkp1  - The normalized matrix representation of E_{k,k+1}
    ...
    for idx = 1:num_matching_patterns
        ...
        for i = 1:k
            ...
            for j = 1:(k+1)
                row_kp1_idx = d - (k + 1) + 1;
                if row_kp1_idx >= 1 && row_kp1_idx <= d
                    if j <= d - row_kp1_idx + 1  
                        lambda_kp1j = pattern(row_kp1_idx, j);
                        l_kp1j = double(lambda_kp1j - j + 1);  
                    else
                        l_kp1j = 0.0; 
                    end
                else
                ...
                numerator = numerator * (l_ki - l_kp1j);
            end
            ...
            if coeff ~= 0.0
                ...
                if is_valid_gt_pattern(new_pattern)
                    ...
                    if isKey(pattern_to_index, key_new)
                        ...
                        adjustment_factor = sqrt(norm_new) / sqrt(norm_current);
    ...
    Ekkp1 = sparse(row_indices, col_indices, values, num_matching_patterns, num_matching_patterns);
end
